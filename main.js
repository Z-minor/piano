...
function loop(){
  ...
  if(向前衝計時>0){
    ...
    if(當前步驟<5){
        ...
        if(答題計時>600){
            document.getElementById('社團標誌').style = `position: absolute;z-index: 1;-webkit-filter: invert(${1-(答題計時-600)/100});filter: invert(${1-(答題計時-600)/100}) ;right:16px;bottom: 16px;transform: scale(0.6);animation: 標誌 10s infinite;`
